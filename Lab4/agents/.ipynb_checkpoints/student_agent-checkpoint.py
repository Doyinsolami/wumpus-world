...
class Agent(Agent):
    ...
    def get_action(self, state, agent_id):
        ...
        # Mark adjacent tiles as visited
        self.mark_adjacent_as_visited(my_pos, adjacent_info)
        ...
        if not self.path:
            self.path = self.bfs_explore(my_pos)

        # Follow the path
        if self.path:
            #print(self.path)
            next_pos = self.path.pop(0)
            direction = self.direction_to_move(my_pos, next_pos)
            tile = adjacent_info.get(direction)
            if tile in ['agent1', 'agent2']:
                #print("Stuck")
                self.path.insert(0, next_pos)
                self.path.insert(0, my_pos)
                return self.move_randomly(adjacent_info,True)
            return direction

        # Fallback: random unvisited direction
        return self.move_randomly(adjacent_info,False)

    def mark_adjacent_as_visited(self, pos, adjacent_info):
        """Mark adjacent tiles as visited if they are empty or occupied by the agent."""
        deltas = {
            'up': (-1, 0),
            'down': (1, 0),
            'left': (0, -1),
        ...
        for direction, tile in adjacent_info.items():
            if tile in ['empty', 'agent1', 'agent2']:  # Mark only empty or agent tiles
                dx, dy = deltas[direction]
                adj_pos = (pos[0] + dx, pos[1] + dy)
                self.visited.add(adj_pos)
    ...
    def move_randomly(self, adjacent_info,force):
        direction_deltas = {
            'up': (-1, 0),
            'down': (1, 0),
            'left': (0, -1),
        ...
        for direction, delta in direction_deltas.items():
            if adjacent_info.get(direction) == 'empty':
                dx, dy = delta
                new_pos = (x + dx, y + dy)
                if (force or new_pos not in self.visited) and new_pos not in self.known_walls:
